...
`Read replicas` are synchronised copies (‘followers’, ‘slaves’) of a primary/leader/master database that are only used only for **read** operations (ie. `SELECT` queries). 
