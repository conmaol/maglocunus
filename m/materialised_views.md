...
Materialised views are a data-architectural pattern that allows you to optimise *read-heavy workloads*, usually for analytical purposes. The trade-off is that materialised may briefly return ‘stale’ data due to refresh lag.
...
```
The query is run and the results stored in a database table. A subsequent query then reads from this stored table.
...
- *complete refresh* – delete old contents, run query again, store new results – slow to compute
- *incremental refresh* – only process rows that have changed – faster to compute but more difficult to implement.
